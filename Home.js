...
function createTiles() {
    ...
    for (let i = 0; i < (length); i++) {
        for (let k=0; k<width; k++) {
            ...
            tile.className = "tile";
            tile.id = "tile"+i.toString()+"."+k.toString();
            tile.addEventListener("click", function() {popupbaraction(i,k);});
...
function envcreate(content) {
    ...
    for (let i=0; i<(array.length-1); i++) {
        ...
        try {
            document.getElementById(seperated[0]).style.backgroundImage=window.getComputedStyle(document.getElementById(seperated[1]), null).getPropertyValue("background-image");
            document.getElementById(seperated[0]).style.backgroundImage
            console.log(window.getComputedStyle(document.getElementById(seperated[1]), null).getPropertyValue("background-image"))
            document.getElementById(seperated[0]).style.backgroundColor=window.getComputedStyle(document.getElementById(seperated[1]), null).getPropertyValue("background-color");
...
function env_to_text() {
    ...
    for (let i=0; i<length; i++) {
        for (let k=0; k<length; k++) {
            ...
            envcont=envcont+tile+"="+document.getElementById(tile).className.split(" ")[1]+"\n"
...
function envsave() {
        ...
        // Ask for file name
        let fileName = prompt("Enter the file name (without extension):", "myTextFile");
...
function createCharacter() {
    ...
    characterpopup.style.backgroundColor = "#"+Math.floor(Math.random()*16777215).toString(16);
    characterpopup.addEventListener("click", function() {toggleCharacterPopup(this.id)});
...
function movecharacter(i,k) {
    ...
    if (target) {
        ...
        const circle = document.getElementById("charcircle"+i.toString()+"."+k.toString())
        ...
        //Removes old circle to place new colour, moves chgaracter
            ...
            if (el.id) {
                ...
                if (normalize(currentColor) === normalize(target.style.backgroundColor)) {
...
function changecolour(i,k) {
    ...
    if (target) {
        console.log("Found element ID:", target.id);
        const tile = document.getElementById("tile"+i.toString()+"."+k.toString())
        ...
        const color_of_tile = window.getComputedStyle(document.getElementById(target.id), null).getPropertyValue("background-color");
        const image_of_tile = window.getComputedStyle(document.getElementById(target.id), null).getPropertyValue("background-image");
